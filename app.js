...
function writeHandles(handles) { localStorage.setItem(STORAGE_KEY, JSON.stringify(handles)); }
function readSettings() {
  try { const raw = localStorage.getItem(SETTINGS_KEY); return raw ? Object.assign({}, DEFAULTS, JSON.parse(raw)) : Object.assign({}, DEFAULTS); }
  catch (e) { return Object.assign({}, DEFAULTS); }
}
function writeSettings(s) { localStorage.setItem(SETTINGS_KEY, JSON.stringify(s)); }

// Render saved handles
...
function renderTweetCard(t) {
  ...
  <article class="tweet-card">
    ...
    <div class="tweet-meta">
      ...
      <div><a class="btn ghost small" target="_blank" rel="noopener" href="${link}">View</a></div>
    </div>
  </article>`;
}
...
function timeSince(d) {
  ...
  const intervals = [{label:'y',sec:31536000},{label:'mo',sec:2592000},{label:'d',sec:86400},{label:'h',sec:3600},{label:'m',sec:60},{label:'s',sec:1}];
  for (const i of intervals){ const val = Math.floor(seconds / i.sec); if (val>=1) return `${val}${i.label} ago`; }
  return 'just now';
}
...
async function loadAndRender() {
  ...
  if (!handles || handles.length === 0) {
    ...
    refreshBtn.disabled = false; refreshBtn.textContent = 'Fetch Now'; isLoading = false; return;
  }
